...
import { DefaultPageBindings } from '../models/DefaultPageBindings'

const { Header } = Navigation

export class Accordion extends PageStack {
  constructor ({ pages = [], offsets = [], renderScrollView } = {}) {
    super(...arguments)
  ...
  onNavigateBack () {
    this._scenes.setState({ hasActivePage: false }, () => {
...
class AccordionScenes extends React.Component {
  ...
  render () {
    ...
    const { activeRoute, renderScrollView, onScroll } = props

    const scrollView = renderScrollView(activeRoute.page.combined)
...
class AccordionScene extends React.Component {
  ...
  render () {
    ...
    const { hasActivePage, page, routerProps } = props
    const { height: windowHeight } = Dimensions.get('window')

    const isActivePage = position === activePosition
    const outputFinal =
        isActivePage ? -(layout.y - contentOffset.y)
    ...
    const state = route.page.combined
    const render = isActivePage
        ? (page.content).bind(page)
        : (page.preview || page.content).bind(page)
    const renderLeftComponent = () => hasActivePage
        ? <Header.BackButton onPress={props.onNavigateBack} />
        : (page.toolbarLeft || (() => null)).bind(page, state)
      ...
      <Animated.View
        ...
        <TouchableOpacity onPress={this.props.onHeaderPress}>
          <AccordionHeader
            ...
            renderRightComponent={page.toolbarRight.bind(page, state)}
            renderTitleComponent={page.toolbarTitle.bind(page, state)}
            />
        </TouchableOpacity>

        <View>
          {render(Object.assign({}, state, { isPreviewPage: !hasActivePage }))}
        </View>
      </Animated.View>
    )
  }
