...
export class Router extends React.Component {
  ...
  render () {
    ...
    const navigator = (
      <Navigator
          ...
          initialRoute={new LinkInformation({ page: initialRoute })}
  ...
  renderScene (route, navigator) {
    ...
    const Page = getPage(routes, route)
    const uri = url.parse(route.page)
    ...
    return (
      <Page {...route.pass} url={uri} ref={page => this._currentPage = page} />
    )
  }
}
...
class RouteMapper {
  ...
  RightButton (route, navigator, index, navState) {
    const Page = getPage(this.routes, route) || {}
