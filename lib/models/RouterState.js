...
export class RouterState {
  ...
  getPage (route = {}) {
    ...
    const SelectedPage = pages[`page:${url.host}`]
    if (SelectedPage == null) {
      throw new Error(`Page at: "${url.host}" does not exist`)
    }

    const PageClass = Object.getPrototypeOf(SelectedPage.prototype)

    if (![ Page, Tabs, Accordion ].some(C => PageClass.constructor !== C)) {
      throw new TypeError(
          `Expected Page at route: ${url.host}. Did your page inherit from ` +
          'the Page component?')
    }
